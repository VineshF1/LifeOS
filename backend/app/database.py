...
from .security import CurrentUser, get_current_user, get_current_user_id
...
engine = create_async_engine(
    _engine_url,
    echo=False,
    # Phase 3: Neon connection throttling — pooled endpoint (:6543) multiplexes,
    # so a small bounded pool per API replica is correct.
    pool_size=5,
    max_overflow=5,
    pool_pre_ping=True,
    pool_recycle=300,
...
# --- Phase 3: dedicated Celery worker engine (NullPool) ---------------------
# One clean connection per task invocation; never share pooled async
# connections across forked worker processes (event-loop collisions).
...
AsyncCelerySession = async_sessionmaker(celery_engine, class_=AsyncSession, expire_on_commit=False)

_SET_TENANT_SQL = text("SELECT set_config('app.current_user_id', :user_id, true)")
_SET_TENANT_EMAIL_SQL = text("SELECT set_config('app.current_user_email', :user_email, true)")
...
async def set_tenant_context(session: AsyncSession, user_id: UUID, user_email: str | None = None) -> None:
    """Pin the RLS tenant for the current transaction only (is_local => true).

    Both id and email are set inside the same transaction: the Phase 2
    shared-access policies read `app.current_user_email`, and without it they
    evaluate to NULL (fail-closed) so sharing silently shows nothing.
    """
    await session.execute(_SET_TENANT_SQL, {"user_id": str(user_id)})
    if user_email:
        await session.execute(_SET_TENANT_EMAIL_SQL, {"user_email": user_email.strip().lower()})
...
async def get_tenant_session_with_email(
    user: CurrentUser = Depends(get_current_user),
) -> AsyncIterator[AsyncSession]:
    """Tenant session that also pins the email for shared-access RLS.

    Use for every route/tool touching documents, chunks, or shares.
    """
    async with AsyncSessionLocal() as session:
        async with session.begin():
            await set_tenant_context(session, user.id, user.email)
            yield session
