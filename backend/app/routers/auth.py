...
from ..security.ratelimit import limit_auth

router = APIRouter(prefix="/auth", tags=["auth"])

_SELECT_USER = text(
    "SELECT id, email, created_at, COALESCE(subscription_tier, 'free') AS subscription_tier, "
    "COALESCE(document_count, 0) AS document_count "
    "FROM users WHERE id = CAST(CAST(:user_id AS text) AS uuid)"
)
...
async def signup(request: Request, payload: SignupRequest, session: AsyncSession = Depends(get_session), _: None = Depends(limit_auth)) -> TokenResponse:
...
async def login(request: Request, payload: LoginRequest, session: AsyncSession = Depends(get_session), _: None = Depends(limit_auth)) -> TokenResponse:
    email = payload.email.strip().lower()
    row = (
        await session.execute(
            text(
