...
PHASE2_PATH = Path(__file__).resolve().parent.parent / "migrations" / "phase2.sql"
PHASE3_PATH = Path(__file__).resolve().parent.parent / "migrations" / "phase3.sql"
...
async def main() -> int:
    ...
    try:
        ...
        if PHASE2_PATH.exists():
            await connection.execute(PHASE2_PATH.read_text(encoding="utf-8"))
            print(f"Phase 2 migration applied ({PHASE2_PATH.name}).")
        if PHASE3_PATH.exists():
            await connection.execute(PHASE3_PATH.read_text(encoding="utf-8"))
            print(f"Phase 3 migration applied ({PHASE3_PATH.name}).")
